...
def average_rating(list1, list2):
    my_dict = dict(zip(list1, list2))
    dict_average = {}
    for key, value in my_dict.items():
        for course, grade in value.items():
            y = sum(grade) / len(grade)
            print(f'Студент {key} имеет среднюю оценку {round((y), 2)} в рамках курса {course}')
            dict_average.setdefault(course, [])
            dict_average[course].append(y)
    for k, v in dict_average.items():
        average_grade = sum(v) / len(v)
        print(f'Средняя оценка в рамках курса {k} - {round((average_grade), 2)}')
...
class Student:
    def __init__(self, name, surname, gender):
        ...
        self.grades = {}
        students.append(self.name)
        all_grades.append(self.grades)
...
class Lecturer(Mentor):
    def __init__(self, name, surname):
        ...
        lecturers.append(self.name)
        grades_lecturers.append(self.grades_lecturer)
...
average_rating(students, all_grades)
average_rating(lecturers, grades_lecturers)
